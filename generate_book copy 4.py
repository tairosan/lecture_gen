...
class BookGenerator:
    ...
    def generate_lecture_content(self, lecture_title, lecture_description):
        ...
        try:
            ...
            print(f"Error generating lecture content: {str(e)}") # エラーハンドリングを追加 ⚠️
            return None
        ...
        for week in self.syllabus:
            ...
            for lecture in week["lectures"]:
                ...
                with open(lecture_file, "w") as f:
                    ...
                    if lecture_content:
                        f.write(lecture_content)  # 講義資料が生成できた場合のみ書き込む 📝
                    else:
                        print(f"Failed to generate lecture content for {lecture['title']}") # 講義資料の生成に失敗した場合はエラーメッセージを表示 ⚠️
                    
                    quiz_content = self.generate_quiz_content(lecture["title"], lecture["description"])
                    if quiz_content:
                        f.write(quiz_content)  # 問題が生成できた場合のみ書き込む ❓
                    else:
                        print(f"Failed to generate quiz content for {lecture['title']}") # 問題の生成に失敗した場合はエラーメッセージを表示 ⚠️
